...
def discover_process_model(df: pd.DataFrame) -> Tuple[str, Optional[str], Optional[bytes]]:
    ...
    try:
        if trace_count <= 100:
            ...
            gviz = hn_visualizer.apply(heu_net, parameters={"format": "dot"})
            if hasattr(gviz, "source"):
                dot = gviz.source
                png_bytes = graphviz.Source(dot).pipe(format='png')
                return algorithm_name, dot, png_bytes
            else:
                return f"Failed to generate DOT from heuristics miner", None, None
...
def main() -> None:
    ...
    st.markdown("Upload an Excel or CSV file containing your event log...")
    ...
    if uploaded_file is not None:
        ...
        st.write(f"Total cases: {df_clean['session_id'].nunique()}")
        if st.button("Proceed to Process Modelling"):
            if pm4py is None:
                st.error("PM4Py is not installed.")
                return
            ...
            if dot:
                ...
                st.subheader("Process Model (Image Only)")
                st.image(image_bytes)
            if image_bytes:
                st.download_button("📥 Download Process Model as PNG",
                    data=image_bytes, file_name="process_model.png", mime="image/png")
